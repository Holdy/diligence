...
const levels = ['trace', '10', 'debug', '20', 'info', '30', 'warn', '40', 'error'];
...
function logTrace(message, params) {
    if (!status.traceActive) return;

    if (params != null) {
        logTarget.trace(message, params);
    } else {
        logTarget.trace(message);
    }
}
...
module.exports.logWarning = logWarn;
