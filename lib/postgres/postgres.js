...
function setStatementTimeoutMS(valueMS) {
    ensure.defined(valueMS, 'setStatementTimeoutMS called with no value');
    defaultParameterMap[k.statement_timeout] = valueMS;
    logging.logDebug(`Postgres statement timeout set to: ${valueMS} ms`);
}
...
async function provideCommandExecutorAsync(credentials, databaseName, usingCallback) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (defaultParameterMap[k.statement_timeout]) {
                librarySpecificCredentials[k.statement_timeout] = defaultParameterMap[k.statement_timeout];
            }
...
module.exports.setStatementTimeoutMS = setStatementTimeoutMS;