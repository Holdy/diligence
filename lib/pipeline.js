...
function from() {
    ...
    pipelineHandle.processEachAsync = async (list, descriptor) => {
        try {
            let itemIndex = 0;
            const descriptorPrefix = descriptor || 'pipeline-input-list';
            for (const listItem of list) {
                const itemDescriptor = `${descriptorPrefix}-${++itemIndex}/${list.length}`;
                logging.logDebug(`processEachAsync() with ${itemDescriptor}`);
                await pipelineHandle.processAsync(listItem, itemDescriptor);
            }
        } catch (errorFromPipeline) {
            logging.logError(errorFromPipeline);
            throw errorFromPipeline;
